...
@section('content')
    ...
    <div class="row">
        <div class="form-three widget-shadow">
            ...
            @endif
            <form name="category" action="{{ url('admin/menuitems/'.$menu->id) }}" method="post" class="form-horizontal">
                @csrf
                <div class="form-group">
                    <label for="focusedinput" class="col-sm-2 control-label">Tên menu</label>
                    <div class="col-sm-8">
                        <input type="text" name="name" class="form-control1" id="focusedinput" value="{{ $menu->name }}" placeholder="Tên menuitems">
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <input type="text" name="slug" class="form-control1" id="focusedinput" value="{{ $menu->slug }}" placeholder="Slug">
                    </div>
                </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('.menu-type').hide(); // k chọn thì ẩn đi
            if($('#'+current_type).length) { // đc chọn thì hiện
                $('#'+current_type).show();
            }
            $('#menu-type').on('change', function () {
                var value = $(this).val(); // lấy giá trị thay đổi
                var type = $(this).find('option:selected').data('type');
                $('.menu-type').hide(); // k chọn thì ẩn đi
                if($('#'+type).length) { // đc chọn thì hiện
                    $('#'+type).show();
                }
            });
        });
